...
var movespeed = 500
var current_speed = 0
const accel = 500  # acceleration rate
...
func _process(delta):
	...
	if Input.is_action_pressed("left"):
		current_speed = max((current_speed) - 40*accel * delta*delta, -movespeed)
		$Sprite2D.flip_h = true
	elif Input.is_action_pressed("right"):
		current_speed = min((current_speed) + 40*accel * delta*delta, movespeed)
